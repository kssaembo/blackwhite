...
export function TeacherPortal({onBack}:{onBack:()=>void}){
 ...
 const [bgmPlaying,setBgmPlaying]=useState(false); const [volume,setVolume]=useState(.45); const [showIntro,setShowIntro]=useState(true); const [notice,setNotice]=useState('');const [endOpen,setEndOpen]=useState(false);
 ...
 return <main className="teacher-page scene-lobby">
  ...
  <section className="teacher-grid teacher-grid-v6">
   ...
   <article className="teacher-card timer-card timer-card-large"><div className="card-title"><b>수업 타이머</b><span>시간 종료 후 학생들에게 전적 제출을 안내하세요.</span></div>{!timerStarted?<div className="timer-input-stage"><input className="timer-big-input" type="number" min="1" max="90" value={minutes} onChange={e=>setMinutes(Math.max(1,Math.min(90,Number(e.target.value)||1)))}/><b>분</b></div>:<div className={`timer-display ${seconds===0?'done':''}`}>{fmt(seconds)}</div>}<div className="timer-row timer-buttons"><button className="primary" onClick={startTimer}>{timerStarted?(timerRunning?'일시정지':'계속'):'시작'}</button><button className="secondary" onClick={resetTimer}>초기화</button></div></article>
  </section>
