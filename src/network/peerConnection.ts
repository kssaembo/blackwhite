...
export class PeerTransport{
  ...
  host(connectionCode:string){this.close();this.role='HOST';this.hostPeerId=`bw-game-${connectionCode}`;this.onStatus('connecting');this.peer=new Peer(this.hostPeerId);this.peer.on('connection',c=>this.attach(c));this.peer.on('error',()=>this.onStatus('error'));}
