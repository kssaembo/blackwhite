...
export class PeerTransport{
  ...
  host(connectionCode:string){
    this.close();this.role='HOST';this.hostPeerId=`bw-game-${connectionCode}`;this.onStatus('connecting');
    return new Promise<void>((resolve,reject)=>{
      let opened=false;
      const peer=new Peer(this.hostPeerId);this.peer=peer;
      const timer=window.setTimeout(()=>{if(opened)return;peer.destroy();this.onStatus('error');reject(new Error('Peer open timeout'));},8000);
      peer.on('open',()=>{opened=true;window.clearTimeout(timer);resolve();});
      peer.on('connection',c=>this.attach(c));
      peer.on('error',(error)=>{this.onStatus('error');if(!opened){window.clearTimeout(timer);reject(error);}});
    });
  }
  join(connectionCode:string){this.close();this.role='JOIN';this.hostPeerId=`bw-game-${connectionCode}`;this.onStatus('connecting');this.peer=new Peer();this.peer.on('open',()=>this.attach(this.peer!.connect(this.hostPeerId,{reliable:true,serialization:'json'})));this.peer.on('error',()=>this.onStatus('error'));}
