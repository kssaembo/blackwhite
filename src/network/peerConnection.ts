...
export class PeerTransport{
  ...
  host(matchId:string){this.close();this.role='HOST';this.hostPeerId=`bw-${matchId.toLowerCase()}`;this.onStatus('connecting');this.peer=new Peer(this.hostPeerId);this.peer.on('connection',c=>this.attach(c));this.peer.on('error',()=>this.onStatus('error'));}
  join(matchId:string){this.close();this.role='JOIN';this.hostPeerId=`bw-${matchId.toLowerCase()}`;this.onStatus('connecting');this.peer=new Peer();this.peer.on('open',()=>this.attach(this.peer!.connect(this.hostPeerId,{reliable:true,serialization:'json'})));this.peer.on('error',()=>this.onStatus('error'));}
