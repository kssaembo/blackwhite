...
export default function App(){
  ...
  const [screen,setScreen]=useState<'HOME'|'CREATE'|'JOIN'|'GAME'|'RECORD'>('HOME');
  ...
  const advanceIfBoth=useCallback((s:MatchState)=>{
    ...
    if(s.round<9){applyMatch(nextRound(s));return;}
    if(s.myScore===s.opponentScore){applyMatch(startSet({...s,set:s.set+1},s.firstPlayerId,s.set+1));setNotice(`연장 ${s.set} 시작`);return;}
    finishGame(s);
  },[finishGame]);
  ...
  const processMessage=useCallback(async(m:GameMessage)=>{
    ...
    if(m.type==='READY'){
      ...
      if(updated.role==='HOST'&&updated.myReady&&updated.opponentReady){const first=Math.random()<.5?updated.playerId:updated.opponentId;send(msg('MATCH_START',updated.matchId,sessionId,1,1,{firstPlayerId:first}));applyMatch(startSet(updated,first,1));}
      return;
    }
    if(m.type==='MATCH_START'){applyMatch(startSet(s,m.payload.firstPlayerId,1));return;}
    if(m.set!==s.set||m.round!==s.round)return;
    if(m.type==='FIRST_COMMIT'){
      if(s.firstPlayerId===s.playerId)return;
      applyMatch({...s,pendingCommitHash:m.payload.commitHash,revealedOpponentColor:m.payload.color,phase:'SECOND_SELECTING'});return;
    }
    if(m.type==='SECOND_SUBMIT'){
      ...
      applyMatch(resolveRound({...s,pendingOpponentTile:m.payload.tile},s.pendingMyTile,m.payload.tile));return;
    }
    if(m.type==='FIRST_REVEAL'){
      ...
      applyMatch(resolveRound(s,s.pendingMyTile,m.payload.tile));return;
    }
  ...
  },[advanceIfBoth,send,sessionId]);

  useEffect(()=>{const t=transportRef.current;t.onMessage=processMessage;t.onStatus=(st)=>{setNetStatus(st);const s=matchRef.current;if(st==='connected'&&s){send(msg('HELLO',s.matchId,sessionId,s.set,s.round,{playerId:s.playerId,playerName:s.playerName}));}};},[processMessage,send,sessionId]);
  ...
  if(screen==='HOME') return <main className="shell home"><header className="brand"><div className="eyebrow">THE GENIUS · CLASS EDITION</div><h1><span>BLACK</span><i>&</i><b>WHITE</b></h1><p>흑과 백</p></header><section className="record-card"><div><small>오늘의 전적</small><strong>{record.wins}승 <em>·</em> {record.losses}패</strong><span>총 {record.games}경기</span></div><div className="points"><small>승점</small><strong>{record.points}</strong></div></section><section className="name-box"><label>내 이름</label><input value={profile.name} maxLength={12} placeholder="이름 또는 별명" onChange={e=>updateName(e.target.value)}/></section><div className="home-actions"><button className="primary" onClick={createGame}>게임 만들기</button><button className="light" onClick={()=>setScreen('JOIN')}>게임 참가</button></div><button className="link" onClick={()=>setScreen('RECORD')}>내 경기 기록</button>{notice&&<div className="toast" onClick={()=>setNotice('')}>{notice}</div>}</main>;

  if(screen==='JOIN') return <main className="shell center"><button className="back" onClick={()=>setScreen('HOME')}>← 돌아가기</button><div className="panel join"><div className="eyebrow">JOIN MATCH</div><h2>게임 참가</h2><button className="scan" onClick={()=>setScanner(true)}>▣ QR 코드 스캔</button><div className="or"><span/>또는<span/></div><label>게임 코드</label><input className="code-input" value={joinCode} onChange={e=>setJoinCode(e.target.value.toUpperCase())} placeholder="BW-K7F2X"/><button className="primary" onClick={()=>joinGame()}>입장</button></div>{scanner&&<QRScanner onCode={(c)=>{setScanner(false);setJoinCode(c);joinGame(c)}} onClose={()=>setScanner(false)}/>} {notice&&<div className="toast" onClick={()=>setNotice('')}>{notice}</div>}</main>;

  if(screen==='CREATE'&&match) return <main className="shell center"><button className="back" onClick={leaveToHome}>← 취소</button><div className="panel create"><div className="eyebrow">CREATE MATCH</div><h2>상대방에게 보여주세요</h2><div className="qr"><QRCodeSVG value={`BWJOIN:${match.matchId}`} size={220} level="M"/></div><div className="match-code">{match.matchId}</div><div className={`connection ${netStatus}`}><span/> {netStatus==='connected'?'상대 연결됨':'상대방을 기다리는 중...'}</div>{netStatus==='connected'&&<button className="primary" onClick={()=>setScreen('GAME')}>READY 화면으로</button>}</div></main>;

  if(screen==='RECORD') return <main className="shell records"><button className="back" onClick={()=>setScreen('HOME')}>← 돌아가기</button><div className="panel"><div className="eyebrow">MY RECORD</div><h2>내 경기 기록</h2><div className="record-summary"><strong>{record.games}경기</strong><span>{record.wins}승 · {record.losses}패 · 승점 {record.points}</span></div><div className="history-list">{record.matches.length===0?<p className="muted">아직 경기 기록이 없습니다.</p>:record.matches.map((m,i)=><div className="history-item" key={m.matchId}><b>{record.matches.length-i}경기</b><span>{m.opponentName}</span><strong className={m.result.toLowerCase()}>{m.result}</strong></div>)}</div><button className="danger" onClick={()=>{if(confirm('경기 기록과 승점을 모두 초기화할까요?\n초기화한 기록은 복구할 수 없습니다.'))setRecord(resetRecord())}}>전적 및 승점 초기화</button></div></main>;
  ...
  if(screen==='GAME'&&match){
    ...
    return <main className="game-shell"><header className="game-top"><div><b>BLACK <i>&</i> WHITE</b><span>{match.set===1?'본게임':`연장 ${match.set-1}`} · ROUND {match.round}/9</span></div><div className={`connection ${netStatus}`}><span/> {netStatus==='connected'?'연결됨':netStatus==='connecting'?'연결 중':'연결 복구 필요'}</div></header><section className="scoreboard"><div><small>나</small><strong>{match.playerName}</strong><b>{match.myScore}</b></div><i>:</i><div><small>상대</small><strong>{match.opponentName||'연결 중'}</strong><b>{match.opponentScore}</b></div></section>
