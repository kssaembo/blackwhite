...
import type { Difficulty, GameType, MatchState, PlayerRecord, Profile, Result, Tile } from './types';
import './styles.css';
...
export default function App(){
  ...
  const finishGame=useCallback((s:MatchState)=>{
    const win=s.myScore>s.opponentScore;audio.play(win?'gameWin':'gameLose');
    const r=saveResult(s.matchId,s.opponentName,win?'WIN':'LOSE',s.gameType,s.difficulty);
  ...
  const processMessage=useCallback(async(m:GameMessage)=>{
    ...
    if(m.type==='HELLO'){
      const updated={...s,opponentId:m.payload.playerId,opponentName:m.payload.playerName,phase:'CONNECTED' as const};applyMatch(updated);
      if(s.role==='HOST'){
        send(msg('HELLO',s.matchId,sessionId,s.set,s.round,{playerId:s.playerId,playerName:s.playerName}));
        send(msg('ROOM_CONFIG',s.matchId,sessionId,s.set,s.round,{gameType:s.gameType,difficulty:s.difficulty,tileSet:s.tileSet}));
      }
      return;
    }
  ...
  const joinGame=(code=joinCode)=>{
    const raw=code.startsWith('BWJOIN:')?code.slice(7):code,clean=raw.replace(/\D/g,'').slice(0,6);
    if(!profile.name.trim()){setNotice('먼저 이름을 입력하세요.');return;}if(!/^\d{6}$/.test(clean)){setNotice('게임 코드는 6자리 숫자입니다.');return;}
  ...
  const reorder=(over:Tile)=>{const s=matchRef.current;if(!s||dragTile===undefined||dragTile===over)return;const arr=[...s.tileOrder],from=arr.indexOf(dragTile),to=arr.indexOf(over);if(from<0||to<0)return;arr.splice(from,1);arr.splice(to,0,dragTile);dragMoved.current=true;applyMatch({...s,tileOrder:arr});};
  ...
  if(screen==='PREVIEW'&&match)return <main className="shell center scene-lobby"><button className="back" onClick={leaveToHome}>← 참가 취소</button><div className="panel room-preview"><div className="eyebrow">ROOM FOUND</div><h2>게임방을 찾았습니다</h2><div className="preview-host">방장 <b>{match.opponentName||'확인 중'}</b></div><div className="room-config-card"><span>게임 유형<strong>{gameTypeLabel(match.gameType)}</strong></span><span>난이도<strong>{difficultyLabel(match.difficulty)}</strong></span></div>{match.gameType==='MIXED'&&<div className="grade-badge">고학년 추천 모드</div>}<p>참가하면 방장과 동일한 9개의 타일을 사용합니다.</p><button className="primary" onClick={()=>setScreen('GAME')}>이 게임에 참가하기</button></div></main>;

  if(screen==='CREATE'&&match)return <main className="shell center scene-lobby"><button className="back" onClick={leaveToHome}>← 취소</button><div className="panel create"><div className="eyebrow">CREATE MATCH</div><h2>상대방에게 보여주세요</h2><div className="room-config-inline"><b>{gameTypeLabel(match.gameType)}</b><span>·</span><b>{difficultyLabel(match.difficulty)}</b></div><div className="qr"><QRCodeSVG value={`BWJOIN:${match.matchId}`} size={220} level="M"/></div><div className="match-code">{match.matchId}</div><div className={`connection ${netStatus}`}><span/> {netStatus==='connected'?'상대 연결됨':'상대방을 기다리는 중...'}</div>{netStatus==='connected'&&<button className="primary" onClick={()=>setScreen('GAME')}>READY 화면으로</button>}</div></main>;

  if(screen==='RECORD')return <main className="shell records scene-main"><button className="back" onClick={()=>setScreen('HOME')}>← 돌아가기</button><div className="panel"><div className="eyebrow">MY RECORD</div><h2>내 경기 기록</h2><div className="record-summary"><strong>{record.games}경기</strong><span>{record.wins}승 · {record.losses}패 · 승점 {record.points}</span></div><div className="history-list">{record.matches.length===0?<p className="muted">아직 경기 기록이 없습니다.</p>:record.matches.map((m,i)=><div className="history-item" key={m.matchId}><b>{record.matches.length-i}경기</b><span>{m.opponentName}<small>{gameTypeLabel(m.gameType??'BASIC')} · {difficultyLabel(m.difficulty??'EASY')}</small></span><strong className={m.result.toLowerCase()}>{m.result}</strong></div>)}</div></div></main>;

  if(screen==='GAME'&&match){
    const canSelect=match.phase==='FIRST_SELECTING'||match.phase==='SECOND_SELECTING',myTurnFirst=match.firstPlayerId===match.playerId,offline=netStatus==='disconnected'||netStatus==='error';
    const mySpec=match.pendingMyTile!==undefined?getSpec(match.tileSet,match.pendingMyTile):undefined;
    return <main className={`game-shell scene-game ${match.phase==='GAME_RESULT'?'scene-result':''}`}><header className="game-top"><div><b>BLACK <i>&</i> WHITE</b><span>{gameTypeLabel(match.gameType)} · {difficultyLabel(match.difficulty)} · {match.set===1?'본게임':`연장 ${match.set-1}`} · ROUND {match.round}/9</span></div><div className="game-top-actions"><div className={`connection ${netStatus}`}><span/> {netStatus==='connected'?'연결됨':netStatus==='connecting'?'연결 중':'연결 복구 필요'}</div><button className="game-reset-btn" onClick={()=>{audio.play('buttonClick');setGameResetOpen(true)}}>게임 종료(게임 초기화)</button></div></header><section className="scoreboard"><div><small>나</small><strong>{match.playerName}</strong><b>{match.myScore}</b></div><i>:</i><div><small>상대</small><strong>{match.opponentName||'연결 중'}</strong><b>{match.opponentScore}</b></div></section>
    {!offline&&(match.phase==='CONNECTED'||match.phase==='READY')&&<section className="ready-panel"><div className="game-mode-pill">{gameTypeLabel(match.gameType)} · {difficultyLabel(match.difficulty)}</div><h2>{match.opponentName?`${match.playerName} VS ${match.opponentName}`:'상대 정보를 확인하는 중...'}</h2><div className="ready-state"><span>나 <b>{match.myReady?'READY ●':'WAITING ○'}</b></span><span>상대 <b>{match.opponentReady?'READY ●':'WAITING ○'}</b></span></div><button className="primary" disabled={match.myReady||!match.opponentName} onClick={ready}>{match.myReady?'READY 완료':'READY'}</button></section>}
    ...
    {!offline&&!['CONNECTED','READY','DISCONNECTED','ROUND_RESULT','GAME_RESULT'].includes(match.phase)&&<><section className="action-stage"><div className="turn-chip">{myTurnFirst?'선 플레이어':'후 플레이어'}</div><div className="big-message">{phaseMessage()}</div>{match.revealedOpponentColor&&match.phase==='SECOND_SELECTING'&&<div className={`opponent-color ${match.revealedOpponentColor.toLowerCase()}`}>{match.revealedOpponentColor}<small>{match.tileSet.filter(t=>t.color===match.revealedOpponentColor).map(t=>t.display).join(' · ')}</small></div>}{match.phase==='FIRST_LOCKED'&&mySpec&&<div className="locked-info">내가 제출한 타일 <b>{mySpec.display}</b> <span>{mySpec.color}</span></div>}</section><section className="tiles"><div className="tiles-heading"><h3>내 타일</h3><small>타일을 드래그하여 나만의 순서로 정렬할 수 있습니다.</small></div><div className="tile-row sortable">{match.tileOrder.map(t=>{const spec=getSpec(match.tileSet,t);return <TileButton key={t} tile={t} spec={spec} disabled={!canSelect||!match.myRemainingTiles.includes(t)} selected={selected===t} onClick={()=>{if(dragMoved.current){dragMoved.current=false;return;}audio.play('tileSelect');setSelected(t)}} onDragStart={(id)=>{dragMoved.current=false;setDragTile(id)}} onDragEnter={reorder} onDragMove={reorderAtPoint} onDragEnd={()=>setDragTile(undefined)}/>})}</div>{canSelect&&<button className="submit" disabled={selected===undefined} onClick={()=>{audio.play('buttonClick');if(selected!==undefined)setConfirmTile(selected)}}>이 타일 제출</button>}</section></>}
    {!offline&&match.phase==='ROUND_RESULT'&&last&&<section className="result-stage"><div className={`result-word ${last.result.toLowerCase()}`}>{resultText(last.result)}</div><div className="result-cards"><div><small>내 타일</small><b>{getSpec(match.tileSet,last.myTile).display}</b><span>{last.myColor}</span></div><div><small>상대 타일</small><b>?</b><span>{last.opponentColor}</span><em>값 비공개</em></div></div><div className="result-score">현재 점수 <b>{match.myScore} : {match.opponentScore}</b></div><button className="primary" disabled={match.myContinue} onClick={continueRound}>{match.myContinue?'상대 확인을 기다리는 중':match.round===9?'게임 종료':'다음 라운드'}</button></section>}
    {match.phase==='GAME_RESULT'&&<section className="gameover"><div className="eyebrow">GAME OVER</div><div className="game-mode-pill">{gameTypeLabel(match.gameType)} · {difficultyLabel(match.difficulty)}</div><h2>{match.myScore>match.opponentScore?'승리!':'패배'}</h2><div className="final-score">{match.myScore} <i>:</i> {match.opponentScore}</div>{match.myScore>match.opponentScore&&<div className="point-up">+1 승점</div>}<div className="mini-record">오늘의 전적 <b>{record.wins}승 · {record.losses}패</b> 승점 <strong>{record.points}</strong></div><button className="primary" onClick={rematch}>다른 상대와 경기하기</button></section>}
    {!['CONNECTED','READY','GAME_RESULT','DISCONNECTED'].includes(match.phase)&&<><nav className="game-side-nav game-side-left"><button onClick={()=>setNotice(match.opponentUsedColors.length?match.opponentUsedColors.map((c,i)=>`R${i+1} ${c}`).join(' · '):'아직 상대 사용 기록이 없습니다.')}>상대 기록</button><button onClick={()=>setNotice(match.history.length?match.history.map(h=>`R${h.round} ${getSpec(match.tileSet,h.myTile).display} ${h.myColor[0]} / ${h.opponentColor} / ${h.result}`).join(' · '):'아직 라운드 기록이 없습니다.')}>라운드 기록</button></nav><nav className="game-side-nav game-side-right"><button className="deduction-side-btn" onClick={()=>setDeductionOpen(true)}>추리판</button></nav></>}
    {deductionOpen&&<DeductionBoard value={match.deduction} tileSet={match.tileSet} onChange={d=>applyMatch({...match,deduction:d})} onClose={()=>setDeductionOpen(false)}/>} {gameResetOpen&&<div className="modal-back"><div className="modal game-confirm-modal"><span className="eyebrow">END GAME</span><h2>현재 게임을 종료할까요?</h2><p>현재 진행 중인 게임 상태가 초기화되고 학생 게임 시작 화면으로 돌아갑니다.</p><div className="modal-actions"><button className="secondary" onClick={()=>setGameResetOpen(false)}>계속 게임하기</button><button className="danger" onClick={()=>{audio.play('buttonClick');setGameResetOpen(false);leaveToHome()}}>게임 종료 및 초기화</button></div></div></div>} {confirmTile!==undefined&&<div className="modal-back"><div className="modal game-confirm-modal"><span className="eyebrow">TILE SUBMIT</span><h2>{getSpec(match.tileSet,confirmTile).display} 타일을 제출할까요?</h2><p>제출하면 이 라운드에서는 변경할 수 없습니다.</p><div className="modal-actions"><button className="secondary" onClick={()=>setConfirmTile(undefined)}>취소</button><button className="primary" onClick={()=>{setConfirmTile(undefined);void submitTile()}}>제출</button></div></div></div>} {notice&&<div className="toast wide" onClick={()=>setNotice('')}>{notice}</div>}{debug&&<aside className="debug"><b>NETWORK DEBUG</b><span>Match {match.matchId}</span><span>Peer {netStatus}</span><span>Sent {transportRef.current.sent}</span><span>Received {transportRef.current.received}</span><span>Reconnect {transportRef.current.reconnects}</span><button onClick={sendPing}>연결 테스트</button></aside>}</main>;
  }
  return null;
}
