...
export default function App(){
  ...
  const joinGame=useCallback((code=joinCode)=>{
    const raw=code.replace(/^BWJOIN:/,'').replace(/^BWREC:/,'').split(':')[0],clean=raw.replace(/\D/g,'').slice(0,6);
  ...
  const showOpponentRecord=()=>{const s=matchRef.current;if(!s)return;const ow=s.history.filter(h=>h.result==='LOSE').length,od=s.history.filter(h=>h.result==='DRAW').length,ol=s.history.filter(h=>h.result==='WIN').length;setDetailNotice({title:'상대 현재 전적',lines:[`${ow}승 ${od}무 ${ol}패`,`현재 경기 ${s.history.length}라운드 완료`]})};
  const showRoundHistory=()=>{const s=matchRef.current;if(!s)return;setDetailNotice({title:'라운드 기록',lines:s.history.length?s.history.map(h=>`${h.round}라운드 · ${resultKo(h.result)}(${colorKo(h.opponentColor)}) · 내 타일 ${getSpec(s.tileSet,h.myTile).display}`):['아직 라운드 기록이 없습니다.']})};
