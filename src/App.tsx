...
export default function App(){
  ...
  const processMessage=useCallback(async(m:GameMessage)=>{
    ...
    if(m.type==='SECOND_SUBMIT'){
      ...
      audio.play('reveal');const resolved=resolveRound({...s,pendingOpponentTile:m.payload.tile},s.pendingMyTile,m.payload.tile);audio.play(resolved.history.at(-1)?.result==='WIN'?'roundWin':resolved.history.at(-1)?.result==='LOSE'?'roundLose':'roundDraw');applyMatch(resolved);return;
    }
    if(m.type==='FIRST_REVEAL'){
      ...
      audio.play('reveal');const resolved=resolveRound(s,s.pendingMyTile,m.payload.tile);audio.play(resolved.history.at(-1)?.result==='WIN'?'roundWin':resolved.history.at(-1)?.result==='LOSE'?'roundLose':'roundDraw');applyMatch(resolved);return;
    }
  ...
  if(screen==='RECORD') return <main className="shell records scene-main"><button className="back" onClick={()=>setScreen('HOME')}>← 돌아가기</button><div className="panel"><div className="eyebrow">MY RECORD</div><h2>내 경기 기록</h2><div className="record-summary"><strong>{record.games}경기</strong><span>{record.wins}승 · {record.losses}패 · 승점 {record.points}</span></div><div className="history-list">{record.matches.length===0?<p className="muted">아직 경기 기록이 없습니다.</p>:record.matches.map((m,i)=><div className="history-item" key={m.matchId}><b>{record.matches.length-i}경기</b><span>{m.opponentName}</span><strong className={m.result.toLowerCase()}>{m.result}</strong></div>)}</div><button className="danger" onClick={()=>{if(confirm('경기 기록과 승점을 모두 초기화할까요?\n초기화한 기록은 복구할 수 없습니다.'))setRecord(resetRecord())}}>전적 및 승점 초기화</button></div></main>;
